...
window.currentRequest = !1, window.currentSearch = !1, jQuery.loadScript = function (t, e) {
    ...
    // location = "/" == location ? location : location.substr(1), $(".guide-icon-menu").find("li").each(function () {
    //     $("a", this).attr("href") != location && $(this).parent().find(".menu-submenu").hide(),
    //     $(this).toggleClass("active", $("a", this).attr("href") == location)
    // }),
    // "series" == $("#router").data("page") ? "film" == $("#router").data("meth")
    // && movies_view() : "series_detail" == $("#router").data("page") && series_view(),
    // "profile" == $("#router").data("page") && profile_view();

    $('ul.guide-icon-menu a').each(function () {
